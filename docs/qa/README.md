...
| `in-browser-agent-tool-roadmap.md` | 浏览器内 AI Agent 还能调用的 Chrome API / DevTools 能力地图 + 风险等级（不写路线图） | 架构决策者、实现者 | 任何 Tool 选型前 |
| `agent-e2e-checklist.md` | Post-MVP 阶段 3 Agent 端到端验收清单（8 场景 + 期望行为 + 失败判定 + 升级触发条件） | 实现者、QA | 任何 Tool 调整前先跑一次 |
...
4. 任何新 Tool 选型前：参考 `in-browser-agent-tool-roadmap.md` 的能力地图 + `docs/constraint/tool-design.md`
5. Agent 行为出现"答非所问"或反复同方案时：跑 `agent-e2e-checklist.md` 8 场景，按"升级触发条件"决定是否加 Tool |
...
- 任何对浏览器内 AI Agent 能力（DOM / JS 执行 / Network / Storage / 截图）的事实陈述，权威段落见 `in-browser-agent-tool-roadmap.md` §1。
...
| 2026-08-21 | 创建 `in-browser-agent-tool-roadmap.md`：浏览器内 AI Agent 能力地图（DOM / JS 执行 / Network / Storage / 截图）+ 风险等级 + 已落地 / 未落地对照；不含"何时做"——路线图 CLAUDE.md 管 | `harness-coding` |
| 2026-08-21 | 创建 `agent-e2e-checklist.md`：Post-MVP 阶段 3 Agent 端到端验收 8 场景 + 失败判定 + 升级触发条件；不在清单的写操作类 Tool（execute_js / click / type / storage / screenshot）需先经 `agent-safety.md` 评审 | `harness-coding` |
